...
function PtRoom() {
  ...
  const userProfileImage = useSelector((state: RootState) => state.profile.profileImage);
  const userOpenViduToken = useSelector((state: RootState) => state.ptLive.userOpenViduToken);
  ...
  const enterSession = (ptLiveId: number) => {
    ...
    axios
      ...
      .then(response => {
        ...
        if (response.status === 200) { // 이미 존재하는 세션
          // 현재 로그인한 유저가 이미 입장했던 세션(토큰이 존재)인지 확인
          if (userOpenViduToken === null) {
            createConnection(ptLiveId);
          }
          response.data.connect(userOpenViduToken);
        } else if (response.status === 404) { // 존재하지 않는 세션
  ...
  const createConnection = (ptLiveId: number) => {
    ...
    axios
      .post(
        OPENVIDU_SERVER_URL + "openvidu/api/sessions/" + ptLiveId + "/connection", data, {
          headers: {
            Authorization: "Basic " + btoa("OPENVIDUAPP:" + OPENVIDU_SERVER_SECRET),
            "Content-Type": "application/json",
          },
