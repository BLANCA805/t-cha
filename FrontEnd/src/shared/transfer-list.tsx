...
export default function TransferList({ times, handleChangeList }: TransferListProps) {
  ...
  const handleToggle = (value: string, index: number) => {
    ...
    if (checked.includes(value)) {
      ...
      if (left.includes(value)){
        if(index + 1 < left.length){
          ...
          if(!checked.includes(left[index+2])){
            setDisabledValueList(disabledValueList => disabledValueList.filter(item => item !==nextButton));
          }
        }
        if(index - 1 >= 0){
          const prevButton = left[index-1];
          if(!checked.includes(left[index-2])){
            setDisabledValueList(disabledValueList => disabledValueList.filter(item => item !==prevButton));
          }
        }
      }
    } else {
      ...
      if (left.includes(value)){
        if(index + 1 < left.length){
          const nextButton = left[index+1];
          setDisabledValueList(disabledValueList => [...disabledValueList, nextButton]);
        }
        if(index - 1 >= 0){
          const prevButton = left[index-1];
          setDisabledValueList(disabledValueList => [...disabledValueList, prevButton]);
        }
      }
    }
  };
  ...
  const handleCheckedLeft = () => {
    ...
    templeft.forEach((value,index) =>{
      ...
      if (rightChecked.includes(value)){
        ...
        if(index-1>=0){
          const stateChangePrevButton = templeft[index-1];
          const IfHalfDiffinRightPrev = rightNotChecked.some(item => isHalfHourDifference(stateChangePrevButton, item));
          if (disabledValueList.includes(stateChangePrevButton)&& !IfHalfDiffinRightPrev){
            setDisabledValueList(disabledValueList => disabledValueList.filter(item => item !== stateChangePrevButton))
          }
        }
        ...
        if(index+1<left.length){
          const stateChangeNextButton = templeft[index+1];
          const IfHalfDiffinRightNext = rightNotChecked.some(item => isHalfHourDifference(stateChangeNextButton, item));
          if (disabledValueList.includes(stateChangeNextButton) && !IfHalfDiffinRightNext){
            setDisabledValueList(disabledValueList => disabledValueList.filter(item => item !== stateChangeNextButton))
          }
        }
      }
      ...
      <Grid item>
        <Grid container direction="column" alignItems="center">
          <Button
            ...
            style={{ backgroundColor: leftChecked.length ? '#6E7783' : '#ebebeb' }}  
          ...
          <Button
            sx={{ my: 0.5 }}
            style={{ backgroundColor: rightChecked.length ? '#6E7783' : '#ebebeb' }}
